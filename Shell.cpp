 ...
 *-------------------------------------------------------------*/
 ...
 void Shell::shell_calback (void(*fptr)(void)) {

  
 }
bool Shell::shell_init(shell_reader_t reader, shell_writer_t writer, char * msg) {
...
bool Shell::shell_register2(int *program (int, char **), const char * string) {
  uint8_t i;
...
bool Shell::shell_register(shell_program_t program, const char * string){
  uint8_t i;
...
void Shell::shell_unregister_all() {
  uint8_t i;
...
void Shell::shell_putc(char c) {
  if (initialized != false && shell_writer != 0)
    shell_writer(c);
}
void Shell::shell_print(const char * string) {
  while (* string != '\0')
    shell_putc(* string++);
}

void Shell::shell_println(const char * string){
  shell_print_pm(PSTR("\r\n"));
}
void Shell::shell_printf(const char * fmt, ...) {
  va_list argl;
  va_start(argl, fmt);
...
void Shell::shell_print_commands(){
  uint8_t i;
...
void Shell::shell_print_error(int error, const char * field){
...
void Shell::shell_print_pm(const char * string) {
  uint8_t c;
  do {
...
void Shell::shell_println_pm(const char * string) {
  shell_print_pm(string);
  shell_print_pm(PSTR("\r\n"));
}
void Shell::shell_printf_pm(const char * fmt, ...) {
  // First copy to RAM
 ...
 *    Internal functions        *
 *-------------------------------------------------------------*/
int Shell::shell_parse(char * buf, char ** argv, unsigned short maxargs) {
  int i = 0;
  int argc = 0;
...
void Shell::shell_process_escape(int argc, char ** argv) {
  uint8_t i, j;
  int sl;
  uint8_t readindex = 0;
...
void Shell::uli2a(unsigned long int num, unsigned int base, int uc, char * bf) {
  int n = 0;
  unsigned int d = 1;
  while (num / d >= base)
    d *= base;
...
void Shell::li2a(long num, char * bf) {
  if (num < 0) {
    num = -num;
    *bf++ = '-';
  }
...
void Shell::ui2a(unsigned int num, unsigned int base, int uc, char * bf) {
  int n = 0;
  unsigned int d = 1;
  while (num / d >= base)
    d *= base;
...
void Shell::i2a(int num, char * bf) {
  if (num < 0) {
    num = -num;
    *bf++ = '-';
  }
...
int Shell::a2d(char ch) {
 ...
 char Shell::a2i(char ch, const char** src, int base, int* nump) {
  const char* p = *src;
  int num = 0;
  int digit;
...
void Shell::putchw(int n, char z, char* bf) {
  char fc = z ? '0' : ' ';
  char ch;
  char* p = bf;
  while (*p++ && n > 0)
...
void Shell::shell_format(const char * fmt, va_list va) {
  char bf[12];
  char ch;
